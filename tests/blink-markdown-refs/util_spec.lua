...
describe("util", function()
  ...
  describe("parse_line", function()
    it("returns none when no @ present", function()
      local result = util.parse_line("no trigger here")
      assert.equal("none", result.mode)
    end)

    it("returns none when space after @", function()
      local result = util.parse_line("text @word more")
      assert.equal("none", result.mode)
    end)

    it("returns normal mode for @query", function()
      local result = util.parse_line("text @query")
      assert.equal("normal", result.mode)
      assert.equal("query", result.query)
      assert.is_nil(result.project)
    end)

    it("returns normal mode for @file#heading", function()
      local result = util.parse_line("text @file#heading")
      assert.equal("normal", result.mode)
      assert.equal("file#heading", result.query)
    end)

    it("returns project_select mode for @!", function()
      local result = util.parse_line("text @!")
      assert.equal("project_select", result.mode)
      assert.equal("", result.query)
      assert.is_nil(result.project)
    end)

    it("returns project_select mode for @!fo", function()
      local result = util.parse_line("text @!fo")
      assert.equal("project_select", result.mode)
      assert.equal("fo", result.query)
    end)

    it("returns project_search mode for @!foo@", function()
      local result = util.parse_line("text @!foo@")
      assert.equal("project_search", result.mode)
      assert.equal("foo", result.project)
      assert.equal("", result.query)
    end)

    it("returns project_search mode for @!foo@bar", function()
      local result = util.parse_line("text @!foo@bar")
      assert.equal("project_search", result.mode)
      assert.equal("foo", result.project)
      assert.equal("bar", result.query)
    end)

    it("returns project_search mode for @!foo@file#heading", function()
      local result = util.parse_line("text @!foo@file#heading")
      assert.equal("project_search", result.mode)
      assert.equal("foo", result.project)
      assert.equal("file#heading", result.query)
    end)
    ...
    it("handles hyphens in project name", function()
      local result = util.parse_line("text @!foo-bar@q")
      assert.equal("project_search", result.mode)
      assert.equal("foo-bar", result.project)
      assert.equal("q", result.query)
    end)

    it("handles underscores in project name", function()
      local result = util.parse_line("text @!my_proj@q")
      assert.equal("project_search", result.mode)
      assert.equal("my_proj", result.project)
      assert.equal("q", result.query)
    end)
  end)
