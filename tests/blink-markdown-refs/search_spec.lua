...
describe("search", function()
  describe("file_item", function()
    it("builds correct file completion item", function()
      ...
      assert.equal("src/main.py", item.label)
      assert.equal(17, item.kind) -- File
      assert.equal("src/main.py", item.insertText)
      assert.equal("file", item.data.type)
    end)

    it("shows relative path as label", function()
      local item = search._file_item("/project/deep/nested/file.lua", "/project", "")
      assert.equal("deep/nested/file.lua", item.label)
    end)
    ...
    it("left-truncates long labels at 30 display columns", function()
      ...
      assert.equal(30, vim.fn.strdisplaywidth(item.label))
  ...
  describe("heading_item", function()
    it("builds correct heading completion item", function()
      ...
      assert.equal("docs/guide.md", item.label)
      assert.equal("Configuration", item.labelDetails.description)
      assert.equal(18, item.kind) -- Reference
      assert.equal("docs/guide.md#configuration", item.insertText)
      assert.equal("heading", item.data.type)
    end)
    ...
    it("shows path on the left and heading text in description", function()
      ...
      assert.equal("README.md", item.label)
      assert.equal("Getting Started", item.labelDetails.description)
    end)
  end)
  ...
  describe("content_item", function()
    it("builds correct content completion item", function()
      ...
      assert.equal("main.py:2:5", item.label)
      assert.equal('print("hello world")', item.labelDetails.description)
      assert.equal(1, item.kind) -- Text
      assert.equal("main.py:2:5", item.insertText)
      assert.equal("content", item.data.type)
    end)
    ...
    it("shows path:line:col as label", function()
      ...
      assert.equal("src/app.js:42:10", item.label)
      assert.equal("const x = 1", item.labelDetails.description)
    end)
  ...
  describe("ranking", function()
    it("gives exact filename matches highest score_offset", function()
      ...
      assert.equal(300, item.score_offset)
    ...
    it("gives non-exact filename matches lower score_offset", function()
      ...
      assert.equal(100, item.score_offset)
    ...
    it("gives exact heading matches mid-high score_offset", function()
      ...
      assert.equal(250, item.score_offset)
    ...
    it("gives non-exact heading matches lower score_offset", function()
      ...
      assert.equal(50, item.score_offset)
    ...
    it("gives non-exact content matches lowest score_offset", function()
      ...
      assert.equal(0, item.score_offset)
  ...
  describe("search", function()
    ...
    it("skips content search for short queries", function()
      ...
      assert.equal(0, #content_items, "expected no content results for single-char query")
    end)
  ...
  describe("heading filtering without #", function()
    ...
    it("returns no headings when query is empty", function()
      ...
      assert.equal(0, #heading_items, "expected no heading items for empty query")
    end)
  ...
  describe("hash syntax", function()
    ...
    it("foo# returns all headings from matched files", function()
      ...
      for _, item in ipairs(heading_items) do
        local filename = vim.fn.fnamemodify(item.data.path, ":t")
        assert.equal("README.md", filename, "heading should be from README.md")
      end
    end)
    ...
    it("suppresses content search when # is present", function()
      ...
      assert.equal(0, #content_items, "expected no content results when # is present")
    end)
    ...
    it("stores heading_query in heading item data.query", function()
      ...
      for _, item in ipairs(heading_items) do
        assert.equal("install", item.data.query, "heading data.query should be the heading portion")
      end
    end)
    ...
    it("returns empty headings when no files match file_query", function()
      ...
      assert.equal(0, #heading_items, "expected no headings when file doesn't match")
    end)
