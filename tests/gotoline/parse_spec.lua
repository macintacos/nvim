...
describe("gotoline.parse", function()
  describe("with no locked file", function()
    ...
    it("rejects line_only with leading zero as filename", function()
      -- "0" alone is not a valid 1-based line; treat as filename
      local r = parse("0", nil)
      assert.equal("filename", r.mode)
      assert.equal("0", r.file_query)
      assert.is_nil(r.line)
    end)
  end)
