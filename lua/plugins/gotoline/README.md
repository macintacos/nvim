...
| `<C-j>` / `<C-n>` / `<Tab>` / `<Down>` | Select next result (wraps). |
| `<C-k>` / `<C-p>` / `<S-Tab>` / `<Up>` | Select previous result (wraps). |
...
| `<CR>` immediately after the `:` (no digits) | Jump to line 1 of the locked file. |
